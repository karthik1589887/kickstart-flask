...
sudo docker compose -f docker/compose/crm.yml --env-file docker/compose/STG.env -p sloopstash-stg-crm up -d --scale app=3 --scale nginx=2'''
    ...
    stage('CD: Execute deployment') {
      steps {
        ...
        ansiblePlaybook(playbook: 'playbook/nginx.yml', credentialsId: 'ansible-node-ssh', inventory: 'inventory/stg', tags: 'setup,update,configure,stop,start', limit: 'sloopstash-stg-crm-nginx-1')
